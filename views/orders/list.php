    ...
    <?php foreach ($orders as $i => $order) { ?>
        ...
        <div class="row mb-5">
            <div class="order p-3 col-12 position-relative">
                ...
                <span class="price position-absolute bottom-0 end-0 py-2 px-3"><?=$order->getTotalPrice()?> €</span>
                <div class="orderbottom d-flex gap-3 flex-wrap">
                    <?php foreach ($containers as $i => $container) {
                        $main = $container->getPartProduct(1);
                        $branch = $container->getPartProduct(2);
                        $drink = $container->getPartProduct(3);
                        $dessert = $container->getPartProduct(4) ?>
                        <div class="ordercontainer container m-0">
