...
class Product {
    ...
    public function isOnSale($currentSales) {
        if (!is_null($currentSales)) {
            foreach ($currentSales as $i => $offer) {
                if ($offer->getScope() == 2) {
                    # Product specific, not entire container
                    if (($offer->getProduct_type() == 0 || $offer->getProduct_type() == $this->id_type) && ($offer->getCategory_affected() == 0 || in_array($offer->getCategory_affected(), $this->getCategories()))) {
                        return $offer;
                    }
                }
            }
    ...
    public function getDiscountedPrice($offer)
    {
        if ($offer->getDiscount_type() == 1) {
            # Base
            return $this->price - $offer->getDiscount();
        } else {
            # Percentage
            return round($this->price*(1 - ($offer->getDiscount() / 100)), 2);
        }
    }
